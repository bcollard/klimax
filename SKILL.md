...
- Registry mirrors: cluster pulls use all of them (containerd `certs.d` per node); `docker pull` on the VM uses the **Docker Hub mirror only**, because dockerd's `registry-mirrors` is Hub-specific. `docker pull quay.io/...` goes direct — that is a Docker limitation, not a klimax bug.
