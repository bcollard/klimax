...
- CPU and memory defaults scale to the Mac: 3/4 of the cores, and half the RAM (more on large machines — a 10-core/32GiB M1 Pro gets 8 CPUs and 20GiB). Disks default to 20GiB root + 30GiB image store. `klimax up` warns if a config asks for more than the machine has — it never blocks, so treat it as guidance to lower the value.
