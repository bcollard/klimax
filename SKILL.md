...
- Defaults are 8 CPUs / 20GiB RAM / 20GiB root disk / 30GiB image disk. `klimax up` warns if a config asks for more CPU, RAM or disk than the Mac has — it never blocks, so treat it as guidance to lower the value.
