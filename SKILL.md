...
- **Local DNS names** (on by default, `network.dns`): every LoadBalancer Service resolves as `<service>.<namespace>.<cluster>.klimax.internal` from the Mac, the VM and every pod. Prefer the name over the VIP in recipes. `klimax dns list` shows what is published; custom names use the `external-dns.kubernetes.io/hostname` annotation (the old `external-dns.alpha.kubernetes.io/` prefix is ignored) and must sit under `<cluster>.klimax.internal`. A new name appears within ~15 s (ExternalDNS sync); wait for `klimax dns list` to show it before the first lookup — macOS caches a "no such name" answer for ~75 s. HTTPS: `network.dns.tls` (on by default) puts a `*.<cluster>.klimax.internal` wildcard in Secret `default/klimax-wildcard-tls` (browser-trusted, covers annotated names and Ingress hosts, not the two-label automatic names — use cert-manager's `klimax-ca` ClusterIssuer for those after `klimax ca attach`); pods trust it by mounting ConfigMap `default/klimax-root-ca`. Fleet members also publish fleet-wide names under `<fleet>.klimax.internal` (annotate: `external-dns.kubernetes.io/hostname: kong-gw.<fleet>.klimax.internal`; first member to publish owns it) with a `*.<fleet>.klimax.internal` wildcard in Secret `default/klimax-fleet-wildcard-tls`. Prefer a fleet-wide name for anything clients should reach regardless of which cluster serves it. Verify from the Mac with `curl` or `dscacheutil -q host -a name <name>` — plain `dig <name>` skips `/etc/resolver` and always fails.
...
- **The host filesystem is not shared by default.** Docker runs inside the VM, so a bind mount resolves there: `docker run -v ~/conf:/etc/app` against an unshared path does **not** fail — dockerd creates the missing directory in the guest and the container gets an **empty** one. Either `klimax copy` the files in and bind the in-VM path, or add the directory to `vm.mounts` in the config (`location:` + `writable: true`), where it appears in the guest at the same absolute path so host-path binds work. `klimax up` applies a `vm.mounts` change by offering to restart the VM (no `destroy` needed); `klimax status` lists the VM's current shares. This applies to `docker compose` from macOS too — the VM's own `docker compose` plugin works, and so does the host `docker-compose` binary against the klimax context.
- Disk filling up (kind node images are ~2.7GB each): `klimax shell df -h /` to check, `klimax prune --dry-run` to find reclaimable caches, `klimax disk resize 40GiB` + `klimax down && klimax up` to grow it.
- CPU and memory defaults scale to the Mac: 3/4 of the cores and half the RAM (a 10-core/32GiB M1 Pro gets 8 CPUs and 16GiB). Disks default to 20GiB root + 30GiB image store. `klimax up` warns if a config asks for more than the machine has — it never blocks, so treat it as guidance to lower the value.
...
- klimax can be used as a plain Docker host, not just for clusters: `klimax docker-context` then `docker ps`. Set `network.disablePortMirroring: false` if you want `-p 8080:80` to land on the Mac's `localhost` the way Docker Desktop does — it is a VM-level setting, so decide before building a lab on it.
- Behind a TLS-intercepting proxy, add its root CA with `vm.caCerts.files` — klimax installs it in the VM, the registry mirrors and every kind node. Without it, pulls fail with `x509: certificate signed by unknown authority` even when the proxy itself is correct.
- Behind a corporate proxy: klimax uses the Mac's system proxy automatically; set `network.proxy` only to override it. `klimax doctor` reports which proxy dockerd is actually using. If shell commands work but image pulls fail, dockerd never got the proxy — `klimax up` writes the systemd drop-in that fixes it.
