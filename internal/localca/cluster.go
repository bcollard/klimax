...
func InstallInCluster(ctx context.Context, g *guest.Client, c *Cluster) (InstallResult, error) {
	var res InstallResult
	dir := "/tmp/klimax-ca-" + c.Name
...
`, res.IssuerNamespace, IssuerName)
	}
...
kubectl --kubeconfig ${KIND_KUBECONFIG} -n %[3]s create configmap %[5]s --from-file=ca.crt=${D}/root.crt \
  ...
  | kubectl --kubeconfig ${KIND_KUBECONFIG} label --local -f - app.kubernetes.io/managed-by=klimax -o yaml \
  | kubectl --kubeconfig ${KIND_KUBECONFIG} apply -f - >/dev/null
%[6]s`, dir, kube, secretNamespace, WildcardSecret, RootConfigMap, issuer)

	slog.Info("Installing local CA material", "cluster", c.Name, "wildcard", c.WildcardNames[0], "certManager", res.IssuerNamespace != "")
	if err := g.RunScript(ctx, fmt.Sprintf("install local CA on cluster %q", c.Name), script); err != nil {
		return res, err
	}
	return res, nil
}
...
func CopySecret(ctx context.Context, g *guest.Client, cluster, namespace string) error {
	script := fmt.Sprintf(`#!/bin/bash
set -euo pipefail
%s
...
`, kubeconfigPrelude(cluster), namespace, secretNamespace, WildcardSecret)
	return g.RunScript(ctx, fmt.Sprintf("copy %s to %s/%s", WildcardSecret, cluster, namespace), script)
}
