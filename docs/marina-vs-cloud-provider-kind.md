  ...
  2. No port-mapping friction — cloud-provider-kind on macOS requires sudo + Envoy containers + port translation. marina's L3 route means zero NAT between host and cluster.
  ...
  4. Self-contained — marina handles the entire stack (VM, Docker, registries, routing, clusters). cloud-provider-kind is a piece you bolt onto an existing setup.
