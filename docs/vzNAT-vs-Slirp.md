SLIRP = Simple Layer Implementation for Receiving Packets — it's user-mode networking for the VM.
  ...
  marina doesn't use it — you use vzNAT (VZNATNetworkDeviceAttachment) instead, which is Apple's native Virtualization.framework NAT and gives the VM a proper lima0 interface with a macOS-assigned IP on bridge1xx.
