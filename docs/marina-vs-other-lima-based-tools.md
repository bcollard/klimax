# marina vs. other Lima-based tools

How marina compares to Rancher Desktop, Colima, and kind-on-lima — the three most common
...
| | marina | Rancher Desktop | Colima | kind-on-lima |
|---|---|---|---|---|
...
| **Host→pod routing** | Pure L3 — no SNAT | Port-mapped (no direct IP) | Port-mapped (no direct IP) | Static route (no SNAT, same as marina) |
...
### Running marina alongside Rancher Desktop

Set `network.disablePortMirroring: true` in marina's config. This disables all Lima TCP port
mirroring for the marina VM (including the kind API-server ports), so Rancher Desktop's
hostagent has no ports to conflict with.

Kubeconfigs for marina clusters will use the VM's direct `lima0` IP instead of `127.0.0.1`.
...
### What marina offers that Rancher Desktop doesn't

- **Multiple kind clusters** — Rancher Desktop ships k3s (one single-node cluster). marina
...
- **Pull-through registry cache** — docker.io, quay.io, gcr.io pulls are served from local
  containers; the cache survives `marina destroy`.
...
### Running marina alongside Colima

Same fix as with Rancher Desktop: set `network.disablePortMirroring: true` in marina's config.
Each tool gets its own Lima VM (Colima's is named `colima` by default, marina's is `marina`),
each on its own vzNAT bridge and IP — no IP conflicts. With port mirroring disabled in the
marina VM, there are no port conflicts on `127.0.0.1` either.
...
Marina is a **multi-cluster Kubernetes lab** — it accepts the constraint of macOS+vzNAT+Docker
...
See [marina-vs-colima.md](marina-vs-colima.md) for a deeper technical comparison.
...
marina. It implements the same core idea — Lima VM + Docker + kind + L3 routing + MetalLB —
...
as marina: a static macOS route for the kind bridge CIDR, and iptables no-SNAT rules in the
guest. The registry mirror containers and MetalLB setup are also direct inspirations for
marina's implementation.

### Running marina alongside kind-on-lima
...
- marina clusters use `https://<lima0IP>:700N` in kubeconfigs (direct VM IP)
- kind-on-lima clusters use `https://127.0.0.1:700N` (Lima port-mirrored)
- No overlap, no conflicts

### What marina improves over kind-on-lima

| Aspect | kind-on-lima | marina |
|---|---|---|
...
| Idempotency | Partial | Full — `marina up` is safe to re-run at any time |
...
**Docker Desktop** does not use Lima. It runs its own Linux VM on Apple
Virtualization.framework with its own port-forwarding mechanism, so there is no vzNAT
bridge conflict with a Lima VM. What *can* collide is `127.0.0.1`: Docker Desktop's port
mappings (including its Kubernetes API server) and Lima's port mirroring both land there.
`disablePortMirroring: true` — the marina default — removes that risk from marina's side.
...
so `disablePortMirroring` does not affect OrbStack. marina and OrbStack coexist without any
special configuration — each VM gets its own distinct `bridge1xx` IP and Docker socket path.
See [marina-vs-orbstack.md](marina-vs-orbstack.md) for a full comparison.
...
additional setup; it does not provide the MetalLB + L3 routing stack marina has.
...
default port mirroring — `disablePortMirroring: true` eliminates this risk on the marina side
regardless of what else is running on the host.

---

## Summary: why marina coexists where others don't
...
Marina solves this with a single config flag. When `disablePortMirroring: true`:

1. Lima's hostagent stops mirroring any TCP port from the marina VM to `127.0.0.1`.
...
The result: marina and any other Lima-based tool can run simultaneously on the same Mac,
